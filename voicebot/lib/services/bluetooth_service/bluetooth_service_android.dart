...
class BluetoothServiceAndroid implements BluetoothServiceInterface {
  ...
  String? _currentConnectedDeviceAddress;
  ...
  BluetoothDevice? get connectedDevice {
    ...
    // Since BluetoothConnection doesn't store the device address,
    // we need to track the connected device separately
    if (_currentConnectedDeviceAddress == null) return null;

    try {
      final device = _bondedDevices.firstWhere(
        (device) => device.address == _currentConnectedDeviceAddress,
      );
      return _convertDevice(device);
    } catch (e) {
      // No device found with matching address
      return null;
    }
  }
  ...
  Future<bool> connectToDevice(BluetoothDevice device) async {
    ...
    try {
      ...
      _isConnected = true;
      _currentConnectedDeviceAddress = device.address;
  ...
  Future<bool> sendCommand(String command) async {
    ...
    try {
      // Convert the command string to bytes and send it
  ...
  @override
  Stream<String>? get dataStream => null; // Not implemented in Android version
  ...
  Future<String?> sendCommandWithResponse(
    ...
    // Not implemented in Android version
  ...
  BluetoothDevice _convertDevice(fbs.BluetoothDevice device) {
    return BluetoothDevice(
      ...
      name: device.name,
      type: _convertDeviceTypeToInt(device.type),
